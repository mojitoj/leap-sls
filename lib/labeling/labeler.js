...
async function label(object) {
  if (object?.resourceType == "Bundle") {
    return labelBundle(object);
  } else {
    const labeledResource = await labelResource(object);
    return sanitizeResource(labeledResource);
  }
}
